...
- **JavaScript execution** � run arbitrary JS in page context (50KB result cap with truncation flag)
- **Batch commands** � multiple commands in one request, with `stopOnWarning` for early abort
- **Wait primitives** � `waitForElement` and `waitForResult` poll until DOM/data is ready
- **Redirect detection** � `expectTitle` on navigate catches silent URL redirects
